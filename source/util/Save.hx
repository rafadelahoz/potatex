...
/**
 * A class to help automate and simplify save functionality.
...
class Save
{
	...
	public function bind(Name:String):Bool
	{
		...
		{
			trace("There was a problem binding to\nthe shared object data from Save.");
			destroy();
			return false;
		}
	 ...
	 */
	private function onDone(Result:SaveStatus):Bool
	{
		switch (Result)
		{
			case SaveStatus.PENDING:
				trace("Save is requesting extra storage space.");
			case SaveStatus.ERROR:
				trace("There was a problem flushing\nthe shared object data from Save.");
			default:
		}
